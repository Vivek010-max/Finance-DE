...
def run_simulation(profile: FinancialProfile, apply_strategy: bool) -> Tuple[int, float]:
    active_debts = [d.model_copy(deep=True) for d in profile.debts]
    total_assets: float = sum(a.current_value for a in profile.assets)
    ...
    months: int = 0
    total_int: float = 0.0
    needs: float = float(profile.rent_mortgage + profile.utilities_bills + profile.food_groceries)
    wants: float = float(profile.lifestyle_luxuries + profile.shopping_miscellaneous)
    total_emi: float = sum(d.monthly_emi for d in profile.debts)
    
    surplus: float = float(profile.monthly_income - needs - wants - total_emi)
    if apply_strategy:
        surplus += float(wants * 0.25) + profile.monthly_extra_buffer
    ...
    while any(d.principal > 0 for d in active_debts) and months < 480:
        ...
        for d in active_debts:
            ...
            interest: float = float(d.principal * (d.annual_roi / 100 / 12))
            total_int += interest  # type: ignore
            d.principal += interest  # type: ignore
            pay: float = min(float(d.principal), float(d.monthly_emi))
            d.principal -= pay  # type: ignore
        
        if fuel > 0:
            for d in active_debts:
                if d.principal > 0:
                    extra: float = float(min(float(d.principal), fuel))
                    d.principal -= extra  # type: ignore
                    fuel -= extra
                    if fuel <= 0: break
...
async def analyze_portfolio(profile: FinancialProfile):
    ...
    return {
        "daily_interest_leak": float(f"{daily_leak:.2f}"),
        "months_saved": std_m - opt_m,
        "interest_saved": float(f"{(std_i - opt_i):.2f}"),
